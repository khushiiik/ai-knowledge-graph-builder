...
from app.tools.comparison_tool import execute_comparison_extraction, clean_ext, normalize_name
...
def test_comparison_with_no_documents(mock_get_provider, db_session):
    result = execute_comparison_extraction(db_session, user_id=1, document_id_str=None, query="compare A and B")
    assert "warning" in result
    assert "empty" in result["warning"]

@patch("app.tools.comparison_tool.get_llm_provider")
@patch("app.tools.comparison_tool.retrieve_chunks")
def test_comparison_with_missing_document_warnings(mock_retrieve, mock_get_provider, db_session):
    ...
    mock_llm.invoke.return_value = MagicMock(content='["Solaris Byte Systems Data", "IT Company Data"]')
...
def test_comparison_successful_matrix(mock_context, mock_retrieve, mock_get_provider, db_session):
    ...
    response2 = MagicMock(content='{"headers": ["Feature / Attribute", "Solaris Report", "IT Report"], "rows": [{"Feature / Attribute": "Employees", "Solaris Report": "100", "IT Report": "200"}]}')
