...
def test_upload_valid_pdf(mock_task, mock_save, client, auth_headers):
    ...
    mock_save.return_value = ("stored_pdf.pdf", "/fake/path/pdf.pdf", 1024, "fake-checksum")
...
def test_upload_valid_docx(mock_task, mock_save, client, auth_headers):
    mock_save.return_value = ("stored_docx.docx", "/fake/path/docx.docx", 2048, "fake-checksum-docx")
    
    files = {"files": ("test2.docx", b"mock docx file contents", "application/vnd.openxmlformats-officedocument.wordprocessingml.document")}
    res = client.post("/documents/upload", files=files, headers=auth_headers)
...
def test_reject_unsupported_file_type(client, auth_headers):
    files = {"files": ("malicious.exe", b"malicious executable bytes", "application/x-msdownload")}
    res = client.post("/documents/upload", files=files, headers=auth_headers)
...
@patch("app.api.routes.document.save_upload_file")
@patch("app.api.routes.document.ingest_document_task")
def test_processing_status_changes_correctly(mock_task, mock_save, client, auth_headers):
